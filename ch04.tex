...
Development tools like Checkstyle (see Appendix~\ref{development}) can warn you about these and other kinds of programming mistakes.
...
A better way to negate logic expressions is to apply one of {\bf De Morgan's laws}:
...
Consider for example a program that prompts the user for a number and computes its logarithm:
...
If so, then we can use \java{nextDouble} without getting an exception.
...
This example checks if the next input is {\em not} a \java{double}, and if so, it displays an error message and quits.
