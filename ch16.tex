...
\java{Drawing} has an \java{ArrayList} of \java{DrawablePolygon} objects.
...
The next block of code creates the \java{Drawing} and adds the polygons to it.

And the final block of code creates a \java{JFrame}, adds the \java{Drawing} to it, and displays the result.

Most of these pieces should be familiar, but there is one part of this program that might surprise you: when we create the \java{RegularPolygon} objects, we assign them to \java{DrawablePolygon} variables, and then pass them to \java{Drawing.add}, which requires \java{DrawablePolygon} objects.
It might not be obvious why that's legal.

\index{IS-A}

But remember that \java{RegularPolygon} extends \java{DrawablePolygon}, so every \java{RegularPolygon} object is also a \java{DrawablePolygon}.

The parameter of \java{Drawing.add} has to be a \java{DrawablePolygon}, but it can be any type of \java{DrawablePolygon}, which means any subclass of \java{DrawablePolygon}, including \java{RegularPolygon}.

\index{polymorphism}

This design is an example of {\bf polymorphism}, a fancy word that means ``having many forms''.
\java{Drawing.add} is a polymorphic function because the parameter can be one of many types, and the \java{ArrayList} in \java{Drawing} is a polymorphic data structure because the elements can be different types.
...
Here's a \java{mainloop} function that runs the animation.
...
Each time through the loop, we call \java{drawing.step} to update the \java{Drawing}; then we sleep with a delay calculated to update 30 times per second.
...
It invokes \java{step} on each \java{DrawablePolygon} in the list and repaints (clears and redraws) the canvas.
...
We'll define a class called \java{BlinkingPolygon} that extends \java{RegularPolygon} and adds two more attributes: \java{visible}, which indicates whether the polygon is visible, and \java{count}, which counts the number of time steps since the last blink.
...
The constructor uses \java{super} to call the \java{RegularPolygon} constructor.  Then it initializes \java{visible} and \java{count}.
...
But the parent class is \java{RegularPolygon}, which does not provide \java{draw}.
In this case, \java{super} invokes \java{draw} from the \java{DrawableRectangle} class, which is the superclass of the superclass.

The \java{step} method increments \java{count}.  Every 10 time steps, it toggles \java{visible} and resets \java{count} to 0.
...
Of course we can, but our \java{Drawing} is currently based on \java{DrawablePolygon}.
...
\item Any class that we want to draw must now extend \java{Actor} or one of its subclasses.
...
Like a class definition, an {\bf interface} definition contains methods, but it contains only the declarations of the methods, not their implementations.
...
% ABD: Is the following true?  I thought they would have "default" protection.  I suggest we make them public explicitly to avoid the issue.
...
And the same is true for subclasses of \java{DrawablePolygon}, so these assignments are legal, too.
...
\java{KeyListener} is an interface for receiving keyboard {\bf events}, which means we can detect and respond to key presses.
...
These methods get invoked when the user presses and releases a key.
...
Here's an implementation of \java{keyPressed} that uses a \java{switch} statement to test which key was pressed and set \java{dx} or \java{dy} accordingly.
...
The constant \java{KeyEvent.VK_UP} is a special code that specifies the up arrow key.
...
Here's an implemenation of \java{keyReleased} that runs when the user releases the key:
...
When the user releases the key, \java{keyReleased} sets \java{dx} or \java{dy} to 0, so the sprite stops moving.
...
Every 25 milliseconds, the \java{Timer} invokes \java{actionPerformed}, which invokes \java{step} on the \java{Drawing}.
...
The \java{Drawing.step} repaints the \java{Canvas}, and the time step is done. 
...
\begin{exercise}
The \java{Polygon} class provides a \java{toString} method, but it does not display the arrays \java{xpoints} and \java{ypoints}, so it is not very useful for debugging.
Before you go on to the more interesting exercises, you might want to write a more useful \java{toString} method for \java{DrawablePolygon}.
\end{exercise}


\begin{exercise}
Write a class called \java{MovingPolygon} that extends \java{RegularPolygon} and implements \java{Actor}.  
It should have instance variables \java{posx} and \java{posy} that specify its position and \java{dx} and \java{dy} that specify its velocity.
During each time step, it should update its position.
If it gets to the edge of the \java{Drawin}, it should reverse direction by changing the sign of \java{dx} or \java{dy}.
\end{exercise}


\begin{exercise}
Modify the \java{VideoGame} so it displays a \java{Sprite} and a \java{MovingPolygon} (from the previous exercise.
...
Hint: you might want to add a method to the \java{Actor} interface, guranteeing that all \java{Actor} objects know how to handle collisions.
\end{exercise}
