...
After that, it initializes the \java{color} attribute to \java{GRAY}.
...
In order for this code to compile, \java{ColorPolygon} needs a \java{step} method too.
...
\item
Any class that we want to draw should now extend \java{Actor} (or one of its subclasses).
...
As such, we can ignore \java{keyTyped}:
...
We begin by adding the following lines to our \java{main} method:
...
The \java{setFocusable} method requests that \java{drawing} will be have the focus initially, without the user having to click on it first.
