...
There is another difference between \java{for} loops and \java{while} loops: if you declare a variable in the initializer, it only exists inside the \java{for} loop.
For example:
...
The inner loop displays an entire row of output, followed by a newline.
...
The argument \java{0} means that we want the letter at {\bf index} 0.
...
The code starts at index 2 (the first \java{'n'}) and finds the next \java{'a'}, which is at index 3.
...
If you give it two different strings that contain the same letters, the condition is \java{false}.
...
And in Unicode, uppercase letters come before lowercase letters.
...
For example, the following method {\em returns} a time string in 12-hour format:
