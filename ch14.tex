...
You now have all the tools you need to make interesting programs.
In this chapter, we show you how to design one in an object-oriented way.
...
But without making changes, neither of them can be used to represent a hand of cards that well.
...
As with the \java{Pile} class, we'll need a way to add cards to the collection.
...
Here is an overloaded version of \java{popCard} that removes and returns the {\em last} card:
...
Like \java{Deck}, the \java{Hand} class extends \java{CardCollection}, so it inherits methods like \java{getLabel}, \java{size}, and \java{getCard}, which are used in \java{display}.
...
If we add the following method to \java{CardCollection}, it will be usable in both subclasses.
...
We will use this method to implement \java{dealAll}, which deals all of the remaining cards.
...
\item Draw a card, reshuffling the discard pile if necessary. (\java{draw})
...
The last method from our bottom-up list is \java{displayState}:
