...
As we saw in Section~\ref{exception}, the error message includes the name of the exception, \java{MissingFormatArgumentException}, and an additional message, \java{Format specifier '%d'}.
That means it doesn't know what value to substitute for \java{\%d}.

The problem is that concatenation happens first, before \java{printf} is invoked.
If the value of \java{inch} is \java{100}, the result of concatenation is \java{"inches=%d100"}.
So \java{printf} gets a format string as an argument, but it doesn't get any values to format.

\index{trace}

The error message also includes a {\bf trace} that shows the method that was running when the error was detected, \java{java.util.Formatter.format}, the method that invoked it, \java{java.io.PrintStream.format}, the method that invoked {\em that}, \java{java.io.PrintStream.printf}, and finally the method you actually wrote, \java{Example.main}.

The last line also names program that was running, \java{Example.java}, and the line it was on, \java{10}.

That's a lot of information, and it includes method names and files names you have no reason to know, at this point.
But don't be overwhelmed.

When you see an error message like this, read the first line carefully, to see what happened.
Then read the last line, to see where it happened.
In some IDEs, you can click on the error message and it will take you to the line of code that was running.
...
%TODO: It looks like we need \% to make LaTeX happy, but it messes up plastex.  Can we make the \java{} environment a pure verbatim env?
