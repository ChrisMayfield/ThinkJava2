...
One possibility is to model the way humans shuffle; for example, we could divide the deck in two halves and then choosing alternately from each one.
...
Helper methods are often \java{private}, because they are only used by methods in the class, and are not needed by methods in other classes.
...
We can do that with \java{subdeck} from Section~\ref{???} and a new method, \java{addDeck}:
...
\item Fill in \java{almostMergeSort}, which divides the deck in half, uses \java{selectionSort} to sort the two halves, and uses \java{merge} to create a new, sorted deck.
