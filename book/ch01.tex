...
In this context, the high-level program is called the {\bf source code}.
...
In order to run a program on different types of machines, it must be compiled multiple times.
...
Instead of translating source code directly into an executable, the Java compiler generates code for a \textbf{virtual machine}.
This ``imaginary'' machine has the functionality common to desktops, laptops, tablets, phones, etc.
Its language, called Java \textbf{byte code}, looks like object code and is easy and fast to interpret.

\index{javac}
\index{JVM}

As a result, it's possible to compile a Java program on one machine, transfer the byte code to another machine, and run the byte code on the other machine.
...
In this example, the first statement does not add a newline, so the output appears on a single line, like this:
