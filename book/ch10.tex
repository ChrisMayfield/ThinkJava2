...
(You might also see them referred to as ``fields'' in the documentation.)
...
The memory diagram in Figure~\ref{fig.aliasing} shows an example.
...
The following code adds 50 to all four sides of the rectangle.
...
You can see this same documentation online by doing a web search for ``Java Point''.
...
The scope of a variable is the area of a program where a variable exists.
...
This example shows three different types of variables:

\begin{enumerate}

\item Local variables (\java{oldv} and \java{newv}), which exist within a method.
When the method returns, these variables are no longer in memory.

\item Parameters (\java{dx} and \java{dy}), which are similar to local variables.
They are initialized with the arguments passed to the method.

\index{this}

\item Attributes (\java{this.x}), which exist as long as the object exists.
They can be used in any of the object's methods and are often referred to using the keyword \java{this} (meaning ``this object'').
...
All we need to change is the variable \java{text} and the body of the \java{for} loop:
...
Programs that manipulate large amounts of text will run much faster if you use \java{StringBuilder}s instead of \java{String}s.
