...
Using dot notation to invoke a method on an object directly, rather than passing the object as a parameter, is more consistent with the style of {\bf object-oriented} programming.
...
Parameters and local variables are created when a method is invoked and deleted when the method returns.  They can be used anywhere inside the method, but not in other methods and not in other classes.

\index{this}

In contrast, attributes are created when an object is created.
They can be used in any of the object's methods via the keyword \java{this}.
And they can be used in other classes via references to the object (e.g., \java{box1.x}).
