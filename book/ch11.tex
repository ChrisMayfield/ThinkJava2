...
The parameter named \java{that} is {\em not} a keyword and could have a different name, but it improves readability.
...
Notice that we compare \java{hour} and \java{minute} with \java{==}, but we compare \java{second} using \java{<} to avoid rounding errors (Section~\ref{rounderr}).
...
Suppose you are going to a movie that starts at 18:50 (i.e., 6:50 PM), and the running time is 2 hours 16 minutes.
...
\item We invoked the method via the \java{startTime} object. (The static method was invoked via the \java{Time} class.)
\end{itemize}
...
In both approaches, \java{startTime} is passed to the method.
But for the non-static method, \java{startTime} is passed implicitly as \java{this}.
