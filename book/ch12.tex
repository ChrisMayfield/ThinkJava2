...
We then use \java{Card}, \java{Deck}, and others to implement the game {\it Crazy Eights}.
...
Class variables are allocated when the program begins (or when the class is used for the first time) and persist until the program ends.
...
We could just invoke \java{System.out.println(Arrays.toString(cards))}.
...
For large values of $n$, binary search is significantly faster.
