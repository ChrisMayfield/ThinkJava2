...
You can read more about it at \url{https://unicode.org/}.
...
Java provides different kinds of loop statements for convenience.
Any \java{for} loop can be rewritten as a \java{while} loop, and vice versa.
We could have printed letters of the alphabet using a \java{while} loop:
...
You might be wondering when to use \java{while} verses when to use \java{for}.
In short, it depends on whether you know how many times you want to repeat.

Another term for ``\java{for} loop'' is ``definite loop'', meaning we know how many times to repeat.
For example, there are 26 letters in the Roman alphabet.
It's better to use a \java{for} loop to print the letters, because we can control the range of values on one line of code.

Another term for ``\java{while} loop'' is ``indefinite loop'', meaning we don't know how many times to repeat.
When validating user input (see Section~\ref{validate}), it's impossible to know how many times the user will enter a wrong value.
...
It's easier to read the \java{Scanner} method calls when they're not combined all on one line of code.
...
The \java{substring} method returns a new string that copies letters from an existing string, given a range of indexes.
...
When comparing strings, it may be tempting to use the \java{==} and \java{!=} operators.
But doing so will almost never work.
The following code compiles and runs, but it always prints \java{"Goodbye!"} regardless what the user types.
...
Even if answer is \java{"yes"}, it will refer to a different object in memory than the literal string \java{"yes"} in the code.
We'll learn more about objects and references in the next chapter.
...
\term{overloading}
A technique of defining two or more methods with the same name but different parameters.
