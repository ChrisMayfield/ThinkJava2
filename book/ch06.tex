...
For example, we could have printed letters of the alphabet using a \java{while} loop:
...
In that case, it's better to use a \java{for} loop, which puts all of the loop control code on one line: \java{for (char c = 'A'; c <= 'Z'; c++)}.
...
And since there is nothing to initialize, a \java{for} loop would be more difficult to understand: \java{for (; !in.hasNextDouble(); word = in.next())}.
...
Even if \java{answer} is \java{"yes"}, it will refer to a different object in memory than the literal string \java{"yes"} from the source code.
