...
Because \java{x} and \java{y} refer to different \java{Integer} objects, this code only displays ``x and y have the same value''.
...
\item \java{isCapitalized(null)} will cause a \java{NullPointerException}, because you can't invoke \java{str.charAt} if \java{str} is \java{null}.

\index{StringIndexOutOfBoundsException}
\index{exception!StringIndexOutOfBounds}

\item \java{isCapitalized("")} will cause a \java{StringIndexOutOfBoundsException}, because there is no character at index \java{0}.
...
The variable \java{str2} is \java{""}, the empty string.
