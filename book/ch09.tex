...
Creates a variable named \java{array} and an array for it to refer to. 
...
Creates a \java{String} variable named \java{word} and a \java{String} object for it to refer to, as shown in Figure~\ref{fig.mem3}.
...
% ABD: I am not sure about this representation of String objects.
% It looks like Strings contain something named `value`, but as
% far as I know, that's not true.

% And do we want to say that String objects are represented as
% character arrays?  It could be an unnecessary source of
% confusion between Strings and arrays.  And it's a level of
% implementation detail we should not have to know.
...
\java{parseInt} throws a \java{NumberFormatException}.
...
% Have we done enhanced for loops?
...
\item If \java{str} is \java{null}, trying to invoke
\java{charAt} will cause a \java{NullPointerException}, because you can't invoke a method on \java{null}.
...
The variable \java{str2} refers to the empty string, \java{""}, represented by a small box.
...
If \java{str} is \java{null}, the \java{||} operator will short circuit and evaluate to \java{true} immediately.
As a result, \java{str.isEmpty()} will not be called.

% Have we mentioned short circuit?
...
% Now that we have short links, should we add one here rather than
