...
It also provides \java{System.out}, which is a special value that has additional methods for displaying output, including \java{println}.
...
If you omit the import statement and later refer to \java{Scanner}, you will get a compiler error saying ``cannot find symbol''.
...
Java applications are typically organized into packages (like \java{java.io} and \java{java.util}) that each include many classes (like \java{PrintStream} and \java{Scanner}).
...
In comparison to English, statements are complete sentences, expressions are phrases, and tokens are individual words and punctuation marks.
...
A better practice is to assign magic numbers to variables with meaningful names, like this:
...
%\hline
%\java{\%s} & character string & \java{"Hello"} \\
\hline
\end{tabular}
...
It might be difficult to see what's wrong, given that \java{inch} and \java{cm} are included in the \java{printf} statement.
But remember that \java{printf} must use commas.
