...
It also provides \java{out}, which is a special value that has additional methods for displaying output, including \java{println}.
...
The source code for these classes is included with the compiler.
...
These methods are not easy to use, but fortunately, Java provides other classes that make it easier to handle common input tasks.
...
This line declares a \java{Scanner} variable named \java{in} and creates a new \java{Scanner} that reads input from \java{System.in}.
...
That means the compiler doesn't know what you mean by the word \java{Scanner}.
...
At this point, we have seen most of the ``language elements'' that make up Java programs.
Figure~\ref{fig.package} shows how these organizational units are related.
...
In the previous example, the tokens are \java{hours}, \java{=}, \java{minutes}, \java{/}, \java{60.0}, and \java{;} (the spaces are ignored).
In plain English, statements are complete sentences, expressions are phrases, and tokens are the individual words and punctuation marks.
...
Note there is a major difference between the Java {\em language}, which defines the {\bf syntax} and {\bf semantics} of the elements in Figure~\ref{fig.package}, and the Java {\em library}, which provides the built-in classes for you to import.
For example, the words \java{PrintStream} and \java{Scanner} are not part of the Java language, but the keywords \java{public} and \java{class} are.
...
A value that appears in a program, like the number 2.54 (or the string \java{" in ="}), is called a {\bf literal}.
...
And if the same value appears many times and might have to change in the future, it makes the code hard to maintain.
...
Variables can vary (hence the name), but the number of centimeters in an inch does not.
...
\hline
\java{\%s} & another string & \java{"Hello"} \\
\hline
\end{tabular}
...
\begin{code}
quotient = 76 / 12;   // quotient
remainder = 76 % 12;  // remainder
\end{code}
...
The modulo operation turns out to be surprisingly useful.
...
Also, many encryption algorithms use the remainder operator extensively.
