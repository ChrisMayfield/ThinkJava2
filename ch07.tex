...
The second makes \java{values} refer to an array of \java{double}s, but the number of elements depends on the value of \java{size} (at the time the array was created).
...
When you create an array of \java{int}s with \java{new}, the elements are initialized to zero.
...
If you actually want to copy the array, not just the reference, you have to create a new array and copy the elements from the old to the new, like this:
...
This line produces the same result shown in Figure~\ref{fig.array4}.
...
Another common pattern is a {\bf search}, which involves traversing an array ``searching'' for a particular element.
For example, the following method takes an array and a target value, and it returns the index where the value appears:
...
For example, the following code searches an array for the value \java{1.23}, which is the third element.
...
Games are an obvious example, but there are many others including scientific simulations.
...
We could have used \java{Arrays.toString} instead, but we like seeing curly braces instead of square brackets.
...
The previous code is repetitive, but it is acceptable as long as the number of ranges is small.
But suppose we wanted to keep track of the number of times each individual score appears.
...
Wait a minute, we can use another array!
...
For each score, we can compute which range it falls in, and then increment the corresponding counter.
...
We also convert the string to lowercase, so that we can treat \java{'A'} and \java{'a'} (for example) as the same latter.
...
Fortunately, Java allows you to perform arithmetic with characters.
...
The enhanced \java{for} loop does not work with strings, but you can convert any string to a character array and iterate that instead:
...
Like in Section~\ref{traversal}, we can return immediately as soon as we determine the result is invalid.
