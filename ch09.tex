...
Other data types, like \java{Integer}, contain numbers and provide methods for manipulating numeric data.
...
In contrast to arrays, objects may {\bf encapsulate} different types of data.
...
By design, all of these classes create immutable objects.
...
For example, here is a 20-digit integer that is too bit to store using a \java{long}.
...
Notice the difference in the previous two examples: you use \java{valueOf} to convert integers, and \java{new} to convert strings.
...
This chapter has introduced two main concepts: objects encapsulate other types of data, and they can be designed to be immutable.
...
The process of encapsulation and generalization (steps 2 and 3) allows you to design as you go along.

Encapsulation and generalization is similar to ``incremental development'' (see Section~\ref{distance}), in the sense that you write a little code and test as you go.
...
The next step is to {\bf encapsulate} this code in a new method.
...
This result is similar to the ``nested loops'' approach in Section~\ref{nested}.
...
We can {\bf generalize} it by replacing the literal \java{6} with a parameter:
