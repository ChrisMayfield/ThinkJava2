...
Other data types, like \java{Integer}, represent numbers and provide methods for manipulating them.
...
When you declare a variable with a primitive type, Java reserves a small amount of memory to store its {\em value}.
...
When drawing memory diagrams, we use an arrow to represent the location of the array (see Figure~\ref{fig.mem2}).
The actual memory location is an integer chosen by Java at run-time.

Objects work in a similar way.
When you declare an object variable, it will store a reference to an object.
The difference is that an object {\bf encapsulates} other data.
...
The variable \java{word} contains the reference to the string.
...
The \java{==} operator, when applied to strings, only compares their references.

\index{null}

In Java, the keyword \java{null} is a special value that means ``no object''.
You can initialize array and object variables this way:
...
In other words, they do not reference anything.
...
If you try to use a variable that is \java{null}, either by invoking a method or accessing an element, Java throws a \java{NullPointerException}.
...
On the other hand, it is legal to pass a \java{null} reference as an argument or receive one as a return value.
\java{null} is often used to represent a special condition or indicate an error.


\section{Strings are immutable}

If the Java library did not have a \java{String} class, we would have to use character arrays to store and manipulate text.
Operations like concatenation (\java{+}), \java{indexOf}, and \java{substring} would be difficult and inconvenient.
Fortunately, Java has a built-in \java{String} class that provides these and other methods.

\index{toUpperCase}
\index{toLowerCase}
\index{immutable}

For example, the methods \java{toUpperCase} and \java{toLowerCase} convert from uppercase to lowercase and vice versa.
These methods are often a source of confusion, because it sounds like they modify strings.
Neither these methods nor any others can change a string, because strings are designed to be {\bf immutable}.

When you invoke \java{toUpperCase} on a string, you get a {\em new} \java{String} object as a result.
For example:
...
%Because neither variable can change the \java{String} object, you don't have to worry about one area of the program accidentally corrupting another area.
...
The wrapper class for \java{char} is \java{Character}; for \java{int} it's \java{Integer}.
...
Wrapper classes provide methods for converting strings to primitive types.
...
The result is the string \java{"12345"}, which as you now know, is stored internally in a character array \java{\{'1', '2', '3', '4', '5'\}}.
...
Now that you know about strings, arrays, and wrapper classes, we can {\em finally} explain the \java{args} parameter (of the \java{main} method) that we have been ignoring since Chapter~\ref{theway}.
...
It's unclear from the output, but the elements of \java{args} are strings.
...
We can prevent this situation by checking \java{args} at the beginning of the program:
...
The \java{return} statement causes the program to terminate.
It's customary for programs that require command-line arguments to display a ``usage'' message when there are none.
