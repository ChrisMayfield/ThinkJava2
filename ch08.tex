...
Algorithms that use iteration are called {\bf iterative}.
They are straight-forward, but sometimes there are more elegant solutions.

In this chapter, we will explore one of the most magical things that an algorithm can do: invoke itself to solve a smaller version of the same problem.
An algorithm that invokes (or refers to) itself is called {\bf recursive}.
...
By convention, the frame for \java{main} is at the top, and the stack grows down.
...
Try this example on your computer -- you might be surprised by all the output!
...
\begin{small}
\begin{quote}
{\bf recursive:} An adjective used to describe a method that is recursive.
\end{quote}
\end{small}
...
We can apply this idea to solve our problem.
...
A great resource for practicing all of these techniques is \href{http://codingbat.com/java}{CodingBat.com}.
...
As you work on these problems, CodingBat will save your code if you create an account.
...
So we will recursively call noX on the substring beginning at index 1.
...
Like in the noX problem, we will only look at one integer at a time.
...
Now go and solve several more CodingBat problems of your own.
