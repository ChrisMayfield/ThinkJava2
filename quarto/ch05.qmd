...
Checkstyle (see Appendix [18.5](#checkstyle){reference-type="ref"
...
-   `!(A && B)`{.java}  is the same as  `!A || !B`{.java}
...
-   `!(x < 5 && y == 3)`{.java}  is the same as 
    `x >= 5 || y != 3`{.java}

-   `!(x >= 1 || y != 7)`{.java}  is the same as 
    `x < 1 && y == 7`{.java}
...
Notice that we didn't have to write  `if (evenFlag == true)`{.java}.
...
Section [5.9](#validate){reference-type="ref" reference="validate"} is
...
Appendix [18.5](#checkstyle){reference-type="ref"
...
Exercise [3.4](#guess){reference-type="ref" reference="guess"}.*
