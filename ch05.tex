...
% TODO: Check with O'Reilly on their style for dashes
...
The value in the \java{switch} statement can be an integer or \java{String}, but not a floating-point number.
...
The expression \java{x < 0 || x > 10} is true if either condition is true, that is, if \java{x} is less than 0 {\em or} greater than 10.

Finally, \java{!(x > 0)} is true if \java{x} is {\em not} greater than 0.
...
In this chapter we have seen relational and logical operators, \java{if} statements, boolean methods, and validating input.
...
Notice how the \java{return} statement allows you to exit a method before you reach the end of it.
...
{\it Hint:} You might want to use \java{Math.pow}.
