...
In this example, \java{main} invokes \java{threeLine}, and \java{threeLine} invokes \java{newLine} (which invokes \java{println}) three times.
...
Beginners wonder why it's worth the trouble to write other methods, when they can do everything in \java{main}.
This example demonstrates a few good reasons:

\begin{itemize}

\item Creating a new method allows you to name a sequence of statements, which makes the code easier to read and understand.
...
Programs always begin running at the first statement of \java{main}, regardless of where it is in the source file.
...
For example, the \java{Math.sin} method takes a numeric argument.
To find the sine of a number, you have to provide the number: \java{Math.sin(0.0)}.
Similarly, the \java{System.out.println} method takes a \java{String}.
...
When you write a method, you name the {\bf parameters}.
...
Another common mistake is to declare the types of the arguments:
...
The \java{hour} in \java{printTime} and the \java{hour} in \java{main} refer to different storage locations, and they can have different values.
...
Learning to trace the execution of a program on paper (or a whiteboard) is a useful skill for communicating with other programmers.
...
Sometimes it is useful to have multiple \java{return} statements, for example, one in each branch of a conditional:
...
The stub includes the method definition and a \java{return} statement:
...
We broke a complex computation into simple methods, tested the methods in isolation, and composed the methods to perform the desired computation.
...
\item For \java{calculateArea(1.0, 2.0, 4.0, 6.0)}, Java uses the first version of \java{calculateArea}, which interprets the arguments as two points.

\item For \java{calculateArea(3.0)}, Java uses the second \java{calculateArea} method that takes a \java{double} as an argument and interprets it as the radius.

\end{itemize}

In the previous example, the second version of \java{calculateArea} invokes the first version (after invoking the \java{distance} method).
...
Answer this question without running the program on a computer.
