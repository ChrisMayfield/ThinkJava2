...
We could just provide \java{getColor} and \java{setColor}, but the code will be more readable if we provide methods more customized for the Game of Life:
...
%CSM - I made this smaller example to match the screenshot
...
If you don't want the program to terminate, you can ``catch'' an exception with a \java{try} statement.
...
If both of the indices are in bounds, the \java{Cell} exists.
...
Now that we've finished the Game of Life, we can using the \java{Cell} and \java{GridCanvas} classes to implement other simulations.
