...
\begin{figure}[!ht]
\begin{center}
\includegraphics[alt={Grid showing a Glider pattern in Conway's Game of Life with five black cells arranged in a specific formation that will move diagonally across the grid}]{figs/glider.png}
...
\includegraphics[trim=0 20 0 20,clip,width=0.95\linewidth,alt={Grid showing a Blinker pattern in Conway's Game of Life with three horizontal black cells that will oscillate between horizontal and vertical orientations}]{figs/figure15-2.png}
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[alt={Diagram showing how a 2D array is structured as an array of arrays, with rows and columns clearly labeled to demonstrate the row-major order storage}]{figs/2D-array.pdf}
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[alt={Screenshot showing the Conway's Game of Life application window with a grid of cells, some alive (black) and some dead (white), displayed in a graphical interface}]{figs/conway.png}
...
%\hline
%\end{tabular}
\includegraphics[height=140pt,alt={Grid diagram showing how cells in the interior have eight neighbors, while cells on edges have five neighbors, and corner cells have only three neighbors}]{figs/grid.pdf}
...
%\end{verbatim}
%\end{minipage}
\includegraphics[trim=0 15 0 15,clip,width=0.93\linewidth,alt={Grid showing the result of countNeighbors method with numbers indicating how many live neighbors each cell has, demonstrating the neighbor counting algorithm for Conway's Game of Life}]{figs/figure15-6.png}
